        ...
        stage('Sonar scan execution') {
            ...
            steps {
                script {
                    def mvnHome = tool 'Maven 3.3.9'
                    withSonarQubeEnv {
                        // sh "'${mvnHome}/bin/mvn'  verify sonar:sonar -Dsonar.host.url=http://sonarserverURl.bc/sonar/ -Dmaven.test.failure.ignore=true"
                    ...
                    if (currentBuild.result == null || currentBuild.result == 'SUCCESS') {
                        ...
                        timeout(time: 2, unit: 'MINUTES') {
                            ...
                            if (developmentArtifactVersion != null && !developmentArtifactVersion.isEmpty()) {
                                def jarName = "alert-manager-${developmentArtifactVersion}.jar"
                                echo "the application is deploying ${jarName}"
                                // NOTE : CREATE your deployemnt JOB, check build job: 'AlertManagerToITT' in jenkins for reference
            ...
            steps {
                ...
                script {
                    ...
                    if (currentBuild.result == null || currentBuild.result == 'SUCCESS') {
                        ...
                        sshagent(['ba6d200d-3b5a-454e-a8e6-c6b6114d3603']) {
                            sh "git tag -f v${v}"
                            sh "git push -f --tags"
                        }
